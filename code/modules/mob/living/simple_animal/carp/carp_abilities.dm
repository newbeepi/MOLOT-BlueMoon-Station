...
/datum/action/innate/lesser_carp_rift/Activate()
    if(!COOLDOWN_FINISHED(src, rift_cooldown))
        owner.balloon_alert(owner, "Способность на перезарядке! Осталось секунд: [round(COOLDOWN_TIMELEFT(src, rift_cooldown)) / 10]!")
        return FALSE
    var/turf/current_location = get_turf(owner)
    var/turf/destination = get_teleport_loc(current_location, owner, max_range)
